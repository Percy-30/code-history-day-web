...
export async function generateMissingEphemeris(dateString: string): Promise<{ event: string, historical_year: number }> {
  ...
  const prompt = `Actúa como historiador de la tecnología extremadamente preciso y riguroso.
Dime un acontecimiento histórico, lanzamiento, hito o nacimiento de un pionero de la informática, software, hardware, internet, videojuegos o tecnología que haya ocurrido EXACTAMENTE y de forma comprobable un ${day} de ${month} (en cualquier año del pasado).
REGLAS CRÍTICAS:
1. NO INVENTES FECHAS. Verifica en tu conocimiento que el evento realmente ocurrió un ${day} de ${month}.
2. Si no estás seguro de la fecha exacta, busca otro evento (aunque sea menor) del cual sí tengas certeza que ocurrió un ${day} de ${month}.
3. PROHIBIDO usar el lanzamiento de Windows 95 (24 de agosto) o eventos genéricos si no coinciden con la fecha exacta.
